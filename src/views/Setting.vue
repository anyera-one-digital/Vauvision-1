...
/** Только цифры из строки (пробелы и нецифровые символы убираем) — как при вводе реквизитов */
const bankFieldDigits = (value: string): string =>
  value.replace(/\s/g, "").replace(/\D/g, "");

/** ИП: те же правила, что в Quiz4.vue */
const validateIpInn = (inn: string): boolean =>
  /^\d{12}$/.test(bankFieldDigits(inn));
const validateIpOgrn = (ogrn: string): boolean =>
  /^\d{15}$/.test(bankFieldDigits(ogrn));
const validateIpBik = (bik: string): boolean =>
  /^\d{9}$/.test(bankFieldDigits(bik));
const validateIpAccount20 = (account: string): boolean =>
  /^\d{20}$/.test(bankFieldDigits(account));
/** Расчётный счёт ИП: как в updateRek2.php (17–24 цифры, не начинается с 301) */
const validateIpRsAccount = (account: string): boolean => {
  const digits = bankFieldDigits(account);
  if (digits.length < 17 || digits.length >= 25) return false;
  if (digits.startsWith("301")) return false;
  return true;
};
const validateIpBankInn = (bankInn: string): boolean =>
  /^\d{9,10}$/.test(bankFieldDigits(bankInn));

/** Физлицо: согласовано с updateRek1.php */
const validateFlBik = (bik: string): boolean =>
  /^\d{9}$/.test(bankFieldDigits(bik));
const validateFlAccountDigitsLen = (account: string): boolean => {
  const n = bankFieldDigits(account).length;
  return n >= 17 && n <= 24;
};
const validateFlFio = (fio: string): boolean => {
  const t = fio.trim();
  if (!t) return false;
  const compact = t.replace(/\s/g, "");
  if (!/^[а-яё-]+$/iu.test(compact)) return false;
...
const validateForm = (field?: string) => {
  ...
  if (!field || field === "nickname") {
    errors.nickname = formData.nickname
      ? ""
      : "Псевдоним обязателен для заполнения";
  }
...
const validateBankDetails = () => {
  ...
  if (bankDetailsType.value === "individual") {
    ...
    const fio = bankDetails.individual.fullName;

    if (!fio?.trim()) indErrors.fullName = "ФИО обязательно";
    else if (!validateFlFio(fio))
      indErrors.fullName =
        "ФИО: только русские буквы и дефис, три слова (как в паспорте)";

    const acc = bankDetails.individual.account;
    if (!acc?.trim()) indErrors.account = "Расчетный счет обязателен";
    else if (!validateFlAccountDigitsLen(acc))
      indErrors.account = "В номере счёта должно быть от 17 до 24 цифр";

    const bik = bankDetails.individual.bik;
    if (!bik?.trim()) indErrors.bik = "БИК обязателен";
    else if (!validateFlBik(bik))
      indErrors.bik = "БИК должен состоять из 9 цифр";

    errors.bankDetails.individual = indErrors;
    return Object.keys(indErrors).length === 0;
  } else {
    const entErrors: Record<string, string> = {};
    const e = bankDetails.entrepreneur;

    if (!e.fullName?.trim()) entErrors.fullName = "ФИО обязательно";
    else if (!validateFlFio(e.fullName))
      entErrors.fullName =
        "ФИО: только русские буквы и дефис, три слова (как в паспорте)";

    if (!e.ogrnip?.trim()) entErrors.ogrnip = "ОГРНИП обязателен";
    else if (!validateIpOgrn(e.ogrnip))
      entErrors.ogrnip = "ОГРН/ОГРНИП должен состоять из 15 цифр";

    if (!e.address?.trim()) entErrors.address = "Адрес обязателен";

    if (!e.inn?.trim()) entErrors.inn = "ИНН обязателен";
    else if (!validateIpInn(e.inn))
      entErrors.inn = "ИНН должен состоять из 12 цифр";

    if (!e.account?.trim()) entErrors.account = "Расчетный счет обязателен";
    else if (!validateIpRsAccount(e.account))
      entErrors.account =
        "Расчётный счёт: 17–24 цифры, не должен начинаться с 301";

    if (!e.bankName?.trim()) entErrors.bankName = "Название банка обязательно";

    if (!e.bankInn?.trim()) entErrors.bankInn = "ИНН банка обязателен";
    else if (!validateIpBankInn(e.bankInn))
      entErrors.bankInn = "ИНН банка: 9–10 цифр";

    if (!e.bik?.trim()) entErrors.bik = "БИК обязателен";
    else if (!validateIpBik(e.bik))
      entErrors.bik = "БИК должен состоять из 9 цифр";

    if (!e.correspondentAccount?.trim())
      entErrors.correspondentAccount = "Корреспондентский счет обязателен";
    else if (!validateIpAccount20(e.correspondentAccount))
      entErrors.correspondentAccount =
        "Корреспондентский счёт должен состоять из 20 цифр";

    if (!e.email) entErrors.email = "Email обязателен";
    else if (!validateEmail(e.email))
...
const submitPasswordChange = async () => {
  if (!formData.email) {
    ElMessage.error(
      "Email не указан. Пожалуйста, заполните email в общих данных",
    );
    closePasswordModal();
    return;
  }
...
const submitEntrepreneurBankDetails = async () => {
  ...
  try {
    const entrepreneurData = {
      ...
      "bank-sp-profile": bankDetails.entrepreneur.bankName,
      "in-bank-sp-profile": bankDetails.entrepreneur.bankInn,
      ...
      if (requisites) {
        ...
        if (region === "Russia") {
          ...
          if (ent) {
            ...
            bankDetails.entrepreneur.bankName = ent.bankName || "";
            bankDetails.entrepreneur.bankInn = ent.bankInn || "";
          ...
          const uf = u.uf as Record<string, unknown> | undefined;
          if (uf) {
            const fromUf = (key: string) => {
              const v = uf[key];
              return v != null && v !== "" ? String(v).trim() : "";
            };
            const bankUf = fromUf("UF_BANK");
            const innBankUf = fromUf("UF_INN_BANK");
            if (!bankDetails.entrepreneur.bankName && bankUf) {
              bankDetails.entrepreneur.bankName = bankUf;
            }
            if (!bankDetails.entrepreneur.bankInn && innBankUf) {
              bankDetails.entrepreneur.bankInn = innBankUf;
            }
          }
...
const scrollToHashAnchor = () => {
  const rawHash = route.hash?.trim();
  if (!rawHash) return;
  const anchor = document.querySelector(rawHash);
...
watch(
  () => [loading.value, route.hash] as const,
  ([isLoading]) => {
    if (!isLoading) {
      scrollToHashAnchor();
    }
          ...
          <div class="setting__content">
              ...
              <div class="setting__photo_flex">
                <div class="setting__photo_image">
                  <img
                    ...
                    alt="Profile"
                  />
                  <div class="setting__photo_img" v-else>
                ...
                <div class="setting__photo_info">
                  <label
                    class="setting__photo_upload button__primary"
                    :class="{ button__disabled: isUploadingImage }"
                  >
                    <span v-if="!isUploadingImage"
                      ><UploadSVG />Загрузить фото</span
                    >
            ...
            <div class="setting__personal">
              ...
              <div class="setting__personal_flex">
                ...
                <div class="form__group">
                  <label for="lastName" class="form__label button"
                    >Фамилия</label
                  >
                  <el-input
                    id="lastName"
            ...
            <div class="setting__general">
              ...
              <div class="setting__general_flex">
                <div class="form__group">
                  <label for="nickname" class="form__label button"
                    >Псевдоним</label
                  >
                  <el-input
                    id="nickname"
            ...
            >
              ...
              <p class="setting__passport_desc">
                Введите данные для отображения в вашем договоре при выкладке
                релиза. Данные вашего паспорта скрыты для просмотра, но вы
                можете обновить их в любое время по кнопке «Изменить».
              </p>
            ...
            >
              <template v-if="isRussia">
                ...
                <div
                  ...
                  v-if="bankDetailsType === 'individual'"
                >
                  <h5 class="setting__details_heading">
                    Банковские данные (Физ. лицо)
                  </h5>
                  <p class="setting__details_description">
                    Заполните реквизиты для выплат на карту физического лица.
                  </p>
                    ...
                    <div class="form__groups">
                      <div class="form__group">
                        ...
                        <el-input
                          ...
                          :class="{
                            error: errors.bankDetails.individual?.fullName,
                          }"
                        ...
                        />
                        <div
                          v-if="errors.bankDetails.individual?.fullName"
                      ...
                      <div class="form__group">
                        ...
                        <el-input
                          ...
                          :class="{
                            error: errors.bankDetails.individual?.account,
                          }"
                        ...
                        />
                        <div
                          v-if="errors.bankDetails.individual?.account"
                          class="error text_very"
                        >
                      ...
                      <div class="form__group">
                        <label for="individualBik" class="form__label button"
                          >БИК банка</label
                        >
                        <el-input
                          id="individualBik"
                        ...
                        />
                        <div
                          v-if="errors.bankDetails.individual?.bik"
                          class="error text_very"
                        >
                ...
                >
                  ...
                  <p class="setting__details_description">
                    Заполните реквизиты для выплат на расчетный счет ИП.
                  </p>
                  <div class="setting__details_flex">
                    <div class="form__labels">
                      <label
                    ...
                    <div class="form__groups">
                      <div class="form__group">
                        <label
                          for="entrepreneurFullName"
                          class="form__label button"
                          >ФИО ПРЕДПРИНИМАТЕЛЯ</label
                        >
                        <el-input
                          id="entrepreneurFullName"
                          v-model="bankDetails.entrepreneur.fullName"
                          :class="{
                            error: errors.bankDetails.entrepreneur?.fullName,
                          }"
                          placeholder="Иванов Иван Иванович"
                          :disabled="isOperationLoading"
                          @blur="validateBankDetails"
                        ...
                        />
                        <div
                          v-if="errors.bankDetails.entrepreneur?.fullName"
                          class="error text_very"
                        >
                      ...
                      <div class="form__group">
                        <label
                          for="entrepreneurAddress"
                          class="form__label button"
                          >Юридический адрес организации</label
                        >
                        <el-input
                          id="entrepreneurAddress"
                          v-model="bankDetails.entrepreneur.address"
                          :class="{
                            error: errors.bankDetails.entrepreneur?.address,
                          }"
                        ...
                        />
                        <div
                          v-if="errors.bankDetails.entrepreneur?.address"
                          class="error text_very"
                        >
                      ...
                      <div class="form__group">
                        <label for="entrepreneurInn" class="form__label button"
                          >ИНН</label
                        >
                        <el-input
                          id="entrepreneurInn"
                          ...
                          :class="{
                            error: errors.bankDetails.entrepreneur?.inn,
                          }"
                        ...
                        />
                        <div
                          v-if="errors.bankDetails.entrepreneur?.inn"
                          class="error text_very"
                        >
                      ...
                      <div class="form__group">
                        ...
                        <el-input
                          ...
                          :class="{
                            error: errors.bankDetails.entrepreneur?.ogrnip,
                          }"
                        ...
                        />
                        <div
                          v-if="errors.bankDetails.entrepreneur?.ogrnip"
                          class="error text_very"
                        >
                      ...
                      <div class="form__group">
                        <label
                          for="entrepreneurAccount"
                          class="form__label button"
                          >Расчётный счёт</label
                        >
                        <el-input
                          id="entrepreneurAccount"
                          v-model="bankDetails.entrepreneur.account"
                          :class="{
                            error: errors.bankDetails.entrepreneur?.account,
                          }"
                          placeholder="408028102544110556789"
                          :disabled="isOperationLoading"
                          @blur="validateBankDetails"
                        ...
                        />
                        <div
                          v-if="errors.bankDetails.entrepreneur?.account"
                          class="error text_very"
                        >
                      ...
                      <div class="form__group">
                        <label
                          for="entrepreneurBankName"
                          class="form__label button"
                          >Банк</label
                        >
                        <el-input
                          id="entrepreneurBankName"
                          v-model="bankDetails.entrepreneur.bankName"
                          :class="{
                            error: errors.bankDetails.entrepreneur?.bankName,
                          }"
                        ...
                        />
                        <div
                          v-if="errors.bankDetails.entrepreneur?.bankName"
                          class="error text_very"
                        >
                          {{ errors.bankDetails.entrepreneur?.bankName }}
                        </div>
                      </div>
                      <div class="form__group">
                        <label
                          for="entrepreneurBankInn"
                          class="form__label button"
                          >ИНН банка</label
                        >
                        <el-input
                          id="entrepreneurBankInn"
                          v-model="bankDetails.entrepreneur.bankInn"
                          :class="{
                            error: errors.bankDetails.entrepreneur?.bankInn,
                          }"
                        ...
                        />
                        <div
                          v-if="errors.bankDetails.entrepreneur?.bankInn"
                          class="error text_very"
                        >
                          {{ errors.bankDetails.entrepreneur?.bankInn }}
                        </div>
                      </div>
                      <div class="form__group">
                        <label for="entrepreneurBik" class="form__label button"
                          >БИК банка</label
                        >
                        <el-input
                          id="entrepreneurBik"
                          ...
                          :class="{
                            error: errors.bankDetails.entrepreneur?.bik,
                          }"
                        ...
                        />
                        <div
                          v-if="errors.bankDetails.entrepreneur?.bik"
                          class="error text_very"
                        >
                      ...
                      <div class="form__group">
                        <label for="entrepreneurKs" class="form__label button"
                          >Корреспондентский счёт банка</label
                        >
                        <el-input
                          id="entrepreneurKs"
                          v-model="
                      ...
                      <div class="form__group">
                        <label
                          for="entrepreneurEmail"
                          class="form__label button"
                          >E-mail предпринимателя</label
                        >
                        <el-input
                          id="entrepreneurEmail"
                          v-model="bankDetails.entrepreneur.email"
                          :class="{
                            error: errors.bankDetails.entrepreneur?.email,
                          }"
                        ...
                        />
                        <div
                          v-if="errors.bankDetails.entrepreneur?.email"
                          class="error text_very"
                        >
              ...
              <div v-else class="setting__details">
                <h5 class="setting__details_heading">Реквизиты для выплат</h5>
                <p class="setting__details_description">
                  Для стран вне РФ: укажите Binance ID / Pay ID, при
                  необходимости — карту РФ и (или) адрес кошелька USDT в сети
                  BEP-20. Достаточно заполнить хотя бы одно поле.
                </p>
                <div class="setting__details_flex">
                  <div class="form__groups">
                    <div class="form__group">
                      ...
                      <el-input
                        ...
                        :class="{
                          error: errors.bankDetails.international?.binancePayId,
                        }"
                      ...
                      />
                      <div
                        v-if="errors.bankDetails.international?.binancePayId"
                        class="error text_very"
                      >
                    ...
                    <div class="form__group">
                      <label for="intlCard" class="form__label button"
                        >Номер карты (РФ), опционально</label
                      >
                      <el-input
                        id="intlCard"
                    ...
                    <div class="form__group">
                      <label for="intlUsdt" class="form__label button"
                        >USDT (BEP-20), опционально</label
                      >
                      <el-input
                        id="intlUsdt"
            ...
            <div class="setting__password">
              ...
              <p class="setting__password_desc">
                Для изменения пароля мы отправим ссылку на вашу электронную
                почту.
              </p>
            ...
            <div class="setting__delete">
              ...
              <p class="setting__delete_desc">
                Если захочешь восстановить аккаунт, можешь написать нам в группу
                VK или Телеграмм.
              </p>
    ...
    >
      <div class="passport-modal">
        <div class="passport-modal__header">
          ...
          <button class="passport-modal__close" @click="closePassportModal">
            ×
          </button>
        </div>
      ...
      <div class="modal modal--small">
        ...
        <div class="modal__content">
          <div class="modal__info">
            <p>
              Ссылка для изменения пароля будет отправлена на вашу электронную
              почту:
            </p>
            <p class="modal__email">{{ formData.email || "не указан" }}</p>
            <p class="modal__hint">
              Письмо придет в течение нескольких минут. Проверьте папку "Спам",
              если не видите письмо во входящих.
            </p>
          </div>
        </div>
